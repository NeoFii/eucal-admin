...
export default function ModelsPage() {
  ...
  useEffect(() => {
    const maxPage = Math.max(1, Math.ceil(filteredVendors.length / PAGE_SIZE));
    if (page > maxPage) {
      setPage(maxPage);
    }
  ...
  const handleDelete = async (event: MouseEvent<HTMLButtonElement>, vendor: Vendor) => {
    event.stopPropagation();
    if (!confirm(`确认删除研发商“${vendor.name}”吗？`)) {
      return;
    }

    try {
      await testingApi.deleteVendor(vendor.id);
      toast.success("删除成功", `研发商“${vendor.name}”已删除`);
      await loadData();
    } catch (error: unknown) {
      console.error("删除研发商失败:", error);
                  ...
                  <CardContent className="flex h-full flex-col gap-5 p-5">
                    ...
                    <div className="mt-auto flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          type="button"
                          onClick={(event) => void handleDelete(event, vendor)}
